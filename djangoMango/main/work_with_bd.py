...
def SELECT(select_name, table, where_status=False, where_column=None, where_data=None):
    ...
    """
    conn = create_connection_mysql_db(db_config["mysql"]["host"], db_config["mysql"]["user"], db_config["mysql"]["pass"], "bnb_music")
    try:
        cursor = conn.cursor() 
        if where_status:
    ...
    **kwargs : Set where_status = True/False, 
    ...
    try:
        ...
        if kwargs.get('where_status'):
            ...
            unit = ("UPDATE "+table+" SET"+data_str+" WHERE "+where_column+"='"+where_data+"'")
            cursor.execute(unit)
            conn.commit()    
